...
const product_repository_1 = __importDefault(require("../../modules/product/product.repository"));
...
async function getAiRecommendation(userMessage) {
    ...
    const productListString = products.data
        .map((p) => `ID: ${p.id}, Nome: "${p.name}", Preço: R$${p.price.toFixed(2)}, Categoria: ${p.subCategories?.[0]?.subCategory?.category?.name}, Subcategorias: ${p.subCategories
